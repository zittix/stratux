...
func sendNetFLARM(msg string) {
	if globalSettings.hasConnectionWithCapability(NETWORK_FLARM_NMEA) {
...
func makeFlarmPFLAAString(ti TrafficInfo) (msg string, valid bool) {
	...
	*/
	...
	msg2 = fmt.Sprintf("PFLAA,%d,%d,,%d,%d,%X,,,,,%d",
		alarmLevel, int16(dist), relativeVertical, idType, ti.Icao_addr+1024, acType) // prototype for bearingless traffic
	for i := range msg {
		checksum = checksum ^ byte(msg[i])
	}
	msg = (fmt.Sprintf("$%s*%02X\r\n", msg, checksum))
